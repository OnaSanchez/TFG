...
def best_corr_target(correlation, target, min_correlation = 0.25):
    ...
    for i in range(len(correlation[target])):
        if abs(correlation[target][i]) > 0.45 and correlation[target][i] != 1:
            l_red.append(i)
...
def plot_metrics(result_dicts):
    ...
    r2_values_norm = r2_values / np.max(r2_values)
    mse_values_norm = mse_values / np.max(mse_values)
    accuracy_values_norm = accuracy_values / np.max(accuracy_values)
    ...
    plt.bar(index - bar_width, r2_values_norm, bar_width, label='R2 Score', color='skyblue')
    plt.bar(index, mse_values_norm, bar_width, label='MSE', color='lightcoral')
