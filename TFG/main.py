...
max_original = merged_df['test_VASS'].max()
min_original = merged_df['test_VASS'].min()
...
regressors = {
    #'LINEAR': perform_regression(norm_merged_df, best_corr, 'LINEAR', show_process=True, plot_pred = False),
    #####'LASSO': perform_regression(norm_merged_df, best_corr, 'LASSO', show_process=True, plot_pred = True),
    #'BAYES': perform_regression(norm_merged_df, best_corr, 'BAYES', show_process=True, plot_pred = False),
    #'RF': best_params_rf(param_sets['RF']['n_estimators'], param_sets['RF']['max_depth'], norm_merged_df, best_corr, True),
    'GRADBOOST': best_params_gb(param_sets['GRADBOOST']['n_estimators'], param_sets['GRADBOOST']['learning_rate'], param_sets['GRADBOOST']['max_depth'], norm_merged_df, best_corr, True),
    #'SVR': best_params_svr(param_sets['SVR']['C'], param_sets['SVR']['epsilon'], norm_merged_df, best_corr, True)
}
...
######################################### PREDS WITH BEST MODEL #####################################


"""
# Suponiendo que 'test_VASS' es la variable que deseas predecir
copia = norm_merged_df.copy()  # Crea una copia del DataFrame original

best_corr = ['PRV_PRVrPHF', 'TEMP_Mean', 'test_STAIs', 'test_ES3']

# Selecciona solo las columnas especificadas en best_corr
X = copia[best_corr].copy()
...
modelo_gb = GradientBoostingRegressor(n_estimators=150, max_depth=5)
...
predicciones_desnorm = predicciones * (max_original - min_original) + min_original

print("Predicciones: ", predicciones_desnorm)
"""
...
regressors = {
    'LINEAR': perform_regression(norm_merged_df, best_corr, 'LINEAR', show_process=True, plot_pred = False),
    ###### 'LASSO': perform_regression(norm_merged_df, best_corr, 'LASSO', show_process=True, plot_pred = False),
    'BAYES': perform_regression(norm_merged_df, best_corr, 'BAYES', show_process=True, plot_pred = False),
    'RF': best_params_rf(param_sets['RF']['n_estimators'], param_sets['RF']['max_depth'], norm_merged_df, best_corr, True),
    'GRADBOOST': best_params_gb(param_sets['GRADBOOST']['n_estimators'], param_sets['GRADBOOST']['learning_rate'], param_sets['GRADBOOST']['max_depth'], norm_merged_df, best_corr, True),
    'SVR': best_params_svr(param_sets['SVR']['C'], param_sets['SVR']['epsilon'], norm_merged_df, best_corr, True)
